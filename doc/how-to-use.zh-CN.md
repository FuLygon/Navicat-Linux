...
1. 从官网下载 Navicat，你应该会得到一个 AppImage 文件，例如 `navicat16-premium-en.AppImage`。

   我假定这个 AppImage 文件在 `~/` 文件夹下。
...
3. [编译 patcher 和 keygen](how-to-build.zh-CN.md)。
...
8. **断开网络**. 找到注册窗口，填写 keygen 给你的 **序列号**，然后点击 `激活`。

9. 通常在线激活会失败，所以在弹出的提示中选择 `手动激活`。

10. 复制 **请求码** 到 keygen，连按两次回车结束。
